...
export const profile = {
  ...
  links: {
    ...
    leetcode: "https://leetcode.com/u/shapoi/",
  },
};
...
export const about = {
  ...
  hobbies: [
    ...
    { icon: "🍜", label: "Eating out & trying new foods" },
    { icon: "🎬", label: "Movies & TV" },
    { icon: "🎮", label: "Gaming (GTA 6 countdown)" },
    { icon: "🏙️", label: "Studying Bay Area Real Estate" },
  ],
};
...
export const contact = [
  ...
  { label: "LeetCode", value: "u/shapoi", href: "https://leetcode.com/u/shapoi/" },
];
...
export const experience = [
  ...
  },
  {
    ...
    bullets: [
      "10 week Summer internship program where I develop backend features for HerbsPro E-commerce web application",
      "Built backend infrastructure: a Python Model Context Protocol (FastMCP) server bridging a MySQL warehouse management system with LLMs, enabling agents to query live stock and write split-shipment pick tickets - automating order routing and cutting manual Warehouse Management System work by 30%",
      "Engineered a cache-aside Redis layer with event-driven invalidation for a 50,000+ SKU catalog, cutting MySQL query volume by 60% and product API response times from 200ms to 90ms."
    ],
  },
  {
    ...
    bullets: [
      "Built and operated an AI data enrichment pipeline that processed 2,409 records across 372 sources, resolving the correct source page for 95.7%.",
      "Deployed vision-language extraction that read 1,084 structured documents directly from photographs - 45% of all records processed - covering data published only as images."
    ],
  }
];
...
// Omit both and the card drops the media column and goes text-only.
export const projects = [
  {
    name: "Swing Stock Analysis Agent",
    period: "September 2026",
    image: "/images/swing-agent.png",
    tags: ["Python", "LangGraph", "Gemini", "PostgreSQL", "pgvector", "BigQuery", "pytest"],
    links: {
      github: "https://github.com/spoigai21/swing-agent",
      pypi: "https://pypi.org/project/swing-agent/",
    },
    bullets: [
      "Open-source CLI agent that detects unusual stock moves, retrieves news published before each move began, and explains the cause — or says \"unexplained\" rather than guessing.",
      "Splits each day's return into market, sector, and company-only moves, then pins the minute the move began from intraday bars — no model is involved until the final judgement, so \"before\" actually means before.",
      "Ingests 28 sources (SEC EDGAR, GDELT via BigQuery, RSS/IR feeds) into PostgreSQL + pgvector, deduplicated with MinHash and embeddings; one combined BigQuery query cut projected scan volume ~99%.",
      "A LangGraph pipeline strips any citation that doesn't resolve to a real retrieved article, and a verdict resting on a dropped citation falls back to \"unexplained\". 412 tests; published via PyPI Trusted Publishing.",
    ],
    note: "The model never gets to invent a story: a placebo test that fed it deliberately unrelated evidence found 0 fabricated explanations in 38 trials — a number re-measured on every release rather than asserted.",
  },
  {
    name: "Fusion Bench",
    period: "July 2026",
    tags: ["CUDA C++", "PyTorch", "Nsight Compute", "pybind11", "NumPy", "Docker"],
    links: { github: "https://github.com/spoigai21/fusion-bench" },
    bullets: [
      "Three progressively optimized FP32 softmax kernels in CUDA C++, benchmarked on an NVIDIA A100.",
      "The final kernel uses online softmax (the FlashAttention primitive), warp-shuffle reductions, float4 loads, and register-resident rows with zero spills.",
      "Runs 1.78× faster than naive at 82% of peak HBM bandwidth, and 1.16× faster than torch.softmax.",
      "Validated against a float64 reference across 14 edge cases; the whole build was verified GPU-free first, so the paid A100 session took ~25 minutes.",
    ],
    note: "Nsight Compute counters show the speedup matches the DRAM traffic reduction to within 2% — and pinpoint where that model breaks: on L2-resident shapes, moving fewer bytes actually ran slower.",
  },
  {
...
//
// Order matters — the grid fills left-to-right, so skills are grouped by family
// (languages → frameworks → infra → cloud/data → AI/ML → analysis → fundamentals
// → quantum → editors). Every entry needs a matching key in lib/skillDetails.js.
export const skills = [
  // languages
  ...
  { name: "LaTeX", logo: "/logos/latex.svg" },
  ...
  // AI / ML — named tools only; the broad umbrella terms (RAG, NLP) are
  // deliberately left off so the grid stays scannable.
  ...
  { name: "Topic Modeling", logo: "/logos/topicmodeling.svg" },
  ...
  // quantum
  { name: "Qiskit", logo: "/logos/qiskit.svg" },
  { name: "IBM Quantum", logo: "/logos/ibmquantum.svg" },
...
export const now = {
  ...
  items: [
    {
      title: "Adorus - adorusjewels.com",
      summary:
        "Implementing social analytics dashboard to assist with marketing.",
    },
    {
      title: "AI projects for Countera",
      summary: "Working on multimodal LLM workflows.",
    },
    {
      title: "Project Work",
      summary:
        "Recently wrapped up Fusion Bench and Swing Stock Analysis",
    },
    { title: "Prepping for NBA Fantasy Draft", mini: true },
